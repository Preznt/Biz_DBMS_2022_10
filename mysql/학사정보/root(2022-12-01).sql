...
/*
힉셍장버(지도교수 정보가 포함된) 데이터를 복제하여 새로운 시트로 생성
...
-- 이 DB에서 학생정보 Table  에는 교수정보가 없다
-- 학생정보 - 학생, 교수 Relation - 교수정보 3개의 table을 묶어서
-- JOIN 을 해야한다


/*
1. 학생테이블에는 교수님 정보가 일체 없다
2. 학생테이블과 학생-교수 Realtion을 JOIN 하여 교수님 정보를 SELECT 한다
3. Relation된 데이터의 교수님 코드와 교수님 테이블을 JOIN 하여
...
Excel 성적데이터의 칼럼 이름을 갖는 임시 테이블 생성하기
*/
CREATE TABLE tbl_성적 (
학번	VARCHAR(5),
국어	INT,
...
-- MySQL 8 이상에서 가능
-- 내부에서 SUM(점수) 를 게산하여 DESC 정렬을 하고
-- 전체 데이터 리스트중에 현재 데이터의 rank 가 얼마인지 보이기
-- 동점자 있을 경우 처리하는 방법에서 dense_rank() 와 비교하여 사용하기
SELECT SUM(점수) AS 총점, AVG(점수) AS 평균,
...
SELECT st_name, SUM(sc_score) AS 총점, AVG(sc_score) AS 평균, st_tel, st_dept, st_grade
FROM tbl_students ST
	LEFT JOIN tbl_scores SC
    ON ST.st_num = SC.sc_stnum
GROUP BY ST.st_num;

SELECT sb_name, SUM(sc_score) AS 총점, AVG(sc_score) AS 평균
FROM tbl_subjects SUB
	LEFT JOIN tbl_scores SC
	ON SC.sc_sbcode = SUB.sb_code
GROUP BY SC.sc_sbcode;
