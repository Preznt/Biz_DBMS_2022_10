...
1. tbl_students 데이터에 대한 문제점 이해하기
	...
	라. 실제 Table 에 다수의 데이터가 UPDATE 되는것은 별로 좋은 방법이 아니다
    마. 자주 발생하는 경우는 아니지만, 학과(이름)정보를 변경하는 경우가
		발생할 가능성이 있다면 이 table 은 정규화 과정을 수행해야 한다
        
    교차상태 마냥 기다리는 상태 시스템 자체가 멈춘것 처럼 보인다
    트랜잭션 모든게 완료되었을 때 OK 가 되는 상황
    중간에 문제가 생기면 rollback 시킨다
    
    예외사항
    원래는 rollback을 시켜야하는데 그러지 않고
*/

UPDATE tbl_students SET st_dept = '건축 아키텍쳐'
WHERE st_dept = '건축공학';
...
-- tbl_student 테이블과 tbl_depts 테이블을 JOIN 을 수행
-- students 테이블의 st_dept(학과명)과 depts 테이블의 dp_name 값이
-- 일치하는 정보를 LEFT JOIN


-- student, dept taable 을 JOIN 한 결과에서 dp_code 데이터를 
...
/*
학생정보의 학과이름을 사용하여 학과테이블에서 학과정보를 JOIN SELECT 하여 보여주기
이 쿼리에서 현제 st_dpcode 데이터는 null 인 상태임이 확인되었다
...
잠시 이 mode를 사용하지 않도록 설정해 주어야 한다
...
2. 나열된 List를 반복하면서 st_dept 칼럼의 데이터를 내부의 () SELECT 에게 한개씩 전달한다
