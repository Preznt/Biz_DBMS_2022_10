...
SELECT country.Name 국가, city.name 도시, city.population 인구
FROM city
INNER JOIN country
...
이러한 상황이 되면 2rodml table 을 select 하여
...
-- 보여지고 있다
-- conutrycode를 가지고 다시 country table에서 국가 정보를 
-- SELECT 해야 한다


-- city table 의 countryCode 데이터와 country table code 데이터가
-- 일치된것을 찾아서 두 테이블 정보를 연결하여 보여라
-- 완전 JOIN(EQ JOIN)
-- 완전 JOIN 주의 사항
-- city table countryCode 데이터가 "반드시" country 테이블의 code 데이터에
-- 존재해야 한다
-- 만약 city 데이터에는 도시에 대한 정보가 있는데
-- 그 city 의 contryCode 에 해당하는 데이터가
...
/*
    ...
    EQ JOIN이라 하고 두 테이블간에 데이터가 완전 일치할 경우 사용
...
예를 들어 성적Table 과 학생Table 을 JOIN 하려고 하는데
성적 Table에는 특정 학번의 성적이 추가되어 있으나
학생 Table에는 특정 학번의 학생정보가 아직 추가되지 않았을 경우
EQ JOIN을 수행하면 성적 데이터가 보이지 않게 된다

두(성적, 학생정보) Table 을 JOIN했을때
성적Table의 모든 데이터는 일단 보여주고
...
-- 5. 없으면 <null> 이라고 보여준다
-- 이 JOIN은 city table의 데이터 검증을 목적으로 한다.
SELECT *
FROM city -- 1
	LEFT JOIN country -- 3
    ON city.CountryCode = country.Code
WHERE city.Population BETWEEN 10000 AND 50000; -- 2
