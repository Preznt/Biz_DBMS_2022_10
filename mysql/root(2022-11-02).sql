...
Database System의 모든 권한을 갖는 사용자이다
...
INSERT tbl_student(st_num,st_name,st_dept,st_grad, st_tel, st_addr)
VALUES('S0001','홍길동','컴퓨터공학', 3, '090-222-2222', '서울특별시');
...
-- 아래 코드는 st_num 값만 있고, st_name 값이 없는 SQL이다
...
-- st_num 칼럼은 PK로 설정되어있다
-- PK 로 설정된 칼럼은 자동으로 UNIQUE  성질을 갖는다
-- 절대!!!!! 중복된 데이터를 저장 할수 없다
...
-- 두번째 방법 : 성춘향 학생의 학번을 S0003 에서 S0033으로 변경(수정)하자
-- DELETE 명령에서 WHERE은 필수다!
SELECT * FROM tbl_student WHERE st_num = 'S0003';

DELETE FROM tbl_student
WHERE st_num = 'S0003';

UPDATE tbl_student -- 1
	SET st_num= 'S0033'-- 3
WHERE st_num= 'S0003'; -- 2
...
-- 학번이(PK)이 st_num 인 학생의 학년, 전화번호, 주소를 변경(Update)
UPDATE tbl_student
SET st_dept = '국어국문학과',
st_grade = 1,
	st_tel = '090-333-3333',
...
-- 학번이 S0033 인 학생의 학번을 S0001로 변경하라
-- 학번이 S0033 인 데이터를 WHERE 하여 학번 데이터를 S0001로 Update
